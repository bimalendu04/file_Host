...
if (elementsArr && elementsArr.length) {
	elementsArr.map((element, index) => {
		...
		element.children[0].style.transition = "all 0.5s";
		element.children[1].style.transition = "all 0.5s";
			...
			if (element.nextSibling.style.height == "" || element.nextSibling.style.height == "auto") {
				...
				elementsArr.map((otherElement, otherIndex) => {
					if (otherIndex !== index) {
						...
						otherElement.nextSibling.style.paddingTop = "0px";
						otherElement.nextSibling.style.paddingBottom = "0px";
						otherElement.nextSibling.style.marginTop = "0px";
						otherElement.nextSibling.style.height = "0px";
					}
				});
			}
		});
